"""
This module provides functionalities to locate and process input files, parse and analyze SARIF
files, extract source and sink code snippets, and generate prompts related to security alerts.

The module is specifically built to integrate with a specific project structure and workflow,
using DAST analysis to aid in vulnerability identification and reporting.

Functions:
- find_input_file: Locates a CSV or XLSX file in the specified input directory.
- read_input_file: Reads the specified input file into a DataFrame.
- extract_code_snippet: Retrieves a snippet of surrounding code for the specified line in a file.
- find_sarif_file: Searches for a SARIF file in a specific directory associated with the given CWE ID.
- parse_sarif_traces: Extracts trace information from a SARIF file.
- format_trace_steps: Formats trace steps into a readable sequence.
- generate_prompts: Create user-readable prompts.
"""
...
def find_sarif_file(cwe_id: str):
    cwe_dir = f"cwe-{cwe_id.strip().lower().zfill(3)}"
