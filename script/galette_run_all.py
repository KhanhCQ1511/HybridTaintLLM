"""
A Distributed Application Security Testing (DAST) pipeline execution module.

This module provides functionality for orchestrating the DAST process through
several predefined steps, including user prompt generation, LLM instrumentation,
tagging overwrite, and propagation testing. Each step is executed in sequence
and ensures proper error handling to terminate on failure.

Functions:
- safe_run: Safely executes a function within a specific step of the pipeline, ensuring errors are managed appropriately.
- dast_pipe: Executes the entire DAST pipeline for a given CWE (Common Weakness Enumeration) ID.
