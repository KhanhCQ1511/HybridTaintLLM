"""
Module for applying taint tracking using Galette's tagging mechanisms.

This module contains sample examples and rules for applying taint tracking
based on specific scenarios involving data sources and sinks. Key examples
illustrate variable tagging and checking mechanisms to ensure tainted data
does not propagate unchecked.

Attributes:
- FEW_SHOT_EXAMPLES: Contains detailed examples and instructions for tagging data sources and verifying sinks in Java code.
- PROMPT_USER_TAGGING_SOURCE: Prompt string with instructions for inserting taint source tagging in Java methods.
- PROMPT_USER_TAGGING_SINK: Prompt string with instructions for inserting taint sink verification in Java methods.
- PROMPT_USER_TAGGING_COMBINED: Instruction set for inserting both source tagging and sink checking in Java methods.
- CWE_INFO: A mapping of CWE identifiers to their descriptions.
"""

FEW_SHOT_EXAMPLES = """\
🔵 SOURCE TAGGING RULES
- Only attach `Tainter.setTag()` to the **variable that truly receives tainted data** (e.g., `param`).
- Insert tag **AFTER** the variable has been fully transformed (decoded, sanitized), not before.
- Do NOT tag wrapper objects or derived variables that don’t directly hold the taint.

🔴 SINK CHECKING RULES
- Always call `Tainter.getTag()` on the **original tainted variable** (param/bar),
NOT on wrapper objects, concatenated strings, or derived variables.
