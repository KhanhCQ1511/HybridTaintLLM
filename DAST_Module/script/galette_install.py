"""
A script to automate processes involving Galette and a specific JDK setup, including cloning, building, and instrumenting
the JDK.

This script automates the following tasks:
- Cloning the Galette repository if not already present.
- Building Galette via Maven.
- Installing the Galette agent into the local Maven repository.
- Downloading and extracting a specific version of Adoptium JDK if it does not already exist.
- Setting up environment variables (JAVA_HOME and MAVEN_OPTS) for later usage.
- Instrumenting the JDK using the Galette Instrument Jar.
"""
