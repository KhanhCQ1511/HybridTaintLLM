"""
A utility module for analyzing dataflow paths in Java projects to detect potential security vulnerabilities.

This module evaluates whether a given dataflow path contains security vulnerabilities based on specific
criteria for sources and sinks. If vulnerabilities are detected, it provides recommended fixes to patch
the issue.

Attributes:
- PROMPT_SYSTEM_TASK: The system-level task formulated for security vulnerability analysis.
- PROMPT_USER: The structure and guidelines issued to a user for analyzing a dataflow path.
- CWE_HINTS: Hints for specific CWEs containing advice and guidance for secure implementation.
"""
